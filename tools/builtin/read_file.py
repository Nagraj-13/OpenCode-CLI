...
class ReadFileParams(BaseModel):
    ...
    limit: int | None = Field(
        None,
        ge=1,
        decription="Maximum number of line to read. If not specified then read the entire file."
    )
    
class ReadFileTool(Tool):
    name = 'read_file'
    description = (
        "Read the content of a file. Returns the file content with line numbers."
        "For large use offset and limit to read specific portions"
    ...
    async def execute(self, invocation:ToolInvocation)-> ToolResult:
        ...
        if not path.exists():
            return ToolResult.error_result(f"File '{path}' does not exist")
        
        if not path.is_file():
            return ToolResult.error_result(f"Path '{path}' is not a file")
        
        file_size = path.stat().st_size
        if file_size > self.MAX_FILE_SIZE:
            return ToolResult.error_result( f"File '{path}' is too large to read, {file_size/(1024*1024):.1f }Mb."
                                            f"Maximum file size is {self.MAX_FILE_SIZE/(1024*1024):.0f}Mb"
                                        )
        ...
        try:
            ...
            if total_lines == 0:
                return ToolResult.success_result("File is empty. No lines to read", metadata={
                    "lines":0
                })
